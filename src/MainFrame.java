...
public class MainFrame extends JFrame {
	...
	public MainFrame() {
		...
		comboBox_search.setModel(new DefaultComboBoxModel<>(
				new String[] {
						"按类别查找",
						"按子类别查找",
						"按书名查找",
		...
		search_button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				cleardatas();
				try {
					int index = comboBox_search.getSelectedIndex();// 获取组合框的选择
					String keyword = search_textField.getText();// 获取文本框
					// 按类别查找","按子类别查找","按书名查找","按作者查找","按价格查找","按编号查找","按介绍查找","按出版日期查找"
					// String sql = "select * from cbook where class='" +SE+ "'";
					String sql1 = "select * from cbook where class like '%" + keyword + "%'";
					String sql2 = "select * from cbook where subclass like '%" + keyword + "%'";
					String sql3 = "select * from cbook where name like '%" + keyword + "%'";
					String sql4 = "select * from cbook where author like '%" + keyword + "%'";
					String sql5 = "select * from cbook where price like '%" + keyword + "%'";
					String sql6 = "select * from cbook where isbn like '%" + keyword + "%'";
					String sql7 = "select * from cbook where introduction like '%" + keyword + "%'";
					String sql8 = "select * from cbook where pubdate like '%" + keyword + "%'";
					switch (index) {
						case 0:
		...
		scrollPane.setBorder(new TitledBorder(UIManager.getBorder("TitledBorder.border"), "显示",
				TitledBorder.LEADING, TitledBorder.TOP, null, new Color(13, 27, 255)));
