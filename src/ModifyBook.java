...
public class ModifyBook extends JFrame {
	 ...
	 * Create the frame.
	 */
	public ModifyBook(final BOOK book) {
		...
		isbn_textField = new JTextField(book.getisbn());
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (checkdatas()) {
					try {
						System.out.println(book.getisbn() + book.getclass1() + book.getsubclass() + book.getname());
						ps = connection.prepareStatement(
								"update cbook set isbn=? ,class=?, subclass=? , name=?,author=?,price=?,pubdate=?,introduction=? WHERE isbn=? ");
